...
% scenario specific 
infile_root_a = [ 'aST_MARv3.9-yearly-' gcm '-' scen ];
...
%    t
    ...
    d1 = ncload([inpath '/aST/' infile_root_a  '-' num2str(time(t)) '.nc']);
    aST = d1.aST;
...
%    figure
    for b = 1:nb
        ...
        eval(['aST_b=aST.*(bas.basin' num2str(b) './bas.basin' num2str(b) ');']);
...
%        plot(sur_b(:),aST_b(:),'.');
        ...
        bint(b,t)=nansum(nansum(aST_b.*af.*mask))*dx*dy;
...
%            [y1,ysel,s1,ssel]= find_local_average(sur_b,aST_b,s0,ds);
            [y1,ysel,s1,ssel]= find_local_median(sur_b,aST_b,s0,ds);
...
% Write netcdf
ancfile = ['../Data/lookup/TaST_' lookup_file '.nc'];
ncwrite_TDSMBTable(ancfile,ss,td,table_out,'aST_ltbl',{'z','b','time'});
